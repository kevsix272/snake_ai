...
from original_game import SnakeEnv #imports the class SnakeEnv. The agent uses this enviroment in training and testing. 
from IPython import display 

# Define model, gets neural  network module from PyTorch. 
class DQN(nn.Module):
    def __init__(self, in_states=111, h1_nodes=256, h2_nodes=128,out_actions=3): 
...
class SnakeDQL():
    ...
    num_states = 115                  # number of states in the environment
    ...
    def train(self, episodes, render=False):
        ...
        # Save policy
        torch.save(policy_dqn.state_dict(), "snake_dql.pt")
        
        plt.figure(1)
        plt.savefig('snake_dql_score.png')
        plt.figure(2)
        plt.savefig('snake_dql_rewards.png')
        plt.show()
    ...
    def test(self, episodes,):
        ...
        policy_dqn.load_state_dict(torch.load("snake_dql.pt", map_location=torch.device('cpu')))
